...
**Status:** pre-alpha, milestone M2 ("depth & danger") complete.
...
`i` inventory, `x` examine, `c` codex, `>`/`<` stairs, `s` save+quit,
`q` quit. A saved run resumes automatically; `--seed` starts fresh.
